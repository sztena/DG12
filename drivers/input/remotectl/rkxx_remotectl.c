...
static struct rkxx_remote_key_table remote_key_table_d17[] = {
    {0xea, KEY_F5},     //power off
    {0x6a, TV_Mouse_Switch},
    {0x28, KEY_MUTE},       //mute
    {0xca, KEY_MENU},
    {0x88, KEY_HOME},     //home
    {0x08, KEY_BACK}, 
    ...
    {0x30, KEY_RIGHT},  ////////
    {0xf2, KEY_DOWN},
	{0xd2, TV_MEDIA_PREVIOUS},
    {0x90, KEY_SEARCH},     //search
    {0x10, KEY_VOLUMEUP},
    {0xe2, TV_MEDIA_NEXT},
    {0xa0, KEY_BACKSPACE},
    {0x20, KEY_VOLUMEDOWN},
...
static struct rkxx_remote_key_table remote_key_table_Egreat[] = {
	...
	{0x38c7, KEY_VOLUMEDOWN},
	{0x7887, TV_MEDIA_PREVIOUS},
	{0xf807, TV_MEDIA_NEXT},
	{0xa857, KEY_VOLUMEUP},
	{0x58a7, KEY_HOME},
	{0x42bd, KEY_BACK},
...
//    },
    {  
       .usercode = 0x40bf, 
       .nbuttons =  17, 
       .key_table = &remote_key_table_d17[0],
    },
    {  
       .usercode = 0x202, 
       .nbuttons =  31, 
...
//kairong mark it: to find  which remote controller you are using
...
{	
    ...
    for (i = 0; i < sizeof(remotectl_button)/sizeof(struct rkxx_remotectl_button); i++){
		printk("Your remote control's usercode is 0x%x\n", ddata->scanData);
        if (remotectl_button[i].usercode == (ddata->scanData&0xFFFF)){
			printk("Find a match usercode in the struct \"remotectl_button[]\"");
            ddata->keybdNum = i;
            return 1;
        }
    }
...
//kairong mark it : when device is suspended, pressing any key in the remote controller, will lead to find out which key on the remote controller is power key.
...
static void remotectl_do_something(unsigned long  data)
{
    ...
    switch (ddata->state)
    {
        ...
        case RMC_USERCODE:
        {
//			printk("USERCODE Before <<=1: ddata->scanData = %x\n", ddata->scanData);
            ddata->scanData <<= 1;
//			printk("USERCODE After <<=1: ddata->scanData = %x\n", ddata->scanData);
            ddata->count ++;

            if ((TIME_BIT1_MIN < ddata->period) && (ddata->period < TIME_BIT1_MAX)){
                ddata->scanData |= 0x01;
//			printk("USERCODE After |= 0x01: ddata->scanData = %x\n", ddata->scanData);
            }
        ...
        case RMC_GETDATA:
        {
//kairong mark it: turn infrared code into integer, so that that the computer can deal with it?
            ddata->count ++;
//			printk("GETDATA Before <<=1: ddata->scanData = %x\n", ddata->scanData);
            ddata->scanData <<= 1;
//			printk("GETDATA After <<=1: ddata->scanData = %x\n", ddata->scanData);

            if ((TIME_BIT1_MIN < ddata->period) && (ddata->period < TIME_BIT1_MAX)){
                ddata->scanData |= 0x01;
//			printk("GETDATA After |= 0x01: ddata->scanData = %x\n", ddata->scanData);
            }           
            if (ddata->count == 0x10){
                printk(KERN_ERR "d=%x\n",(ddata->scanData&0xFFFF));
//kairong 20131221 mark it: to check if the scan is valid (means complimentary, such as 0x728d ---> 7+8=f,2+d=f), if it's valid, then go to find a match keycode (to confirm which key you have pressed), if not, go to RMC_PRELOAD.
                    ...
                    if (remotectl_keycode_lookup(ddata)){
                      ...
                      //kairong20131221
                         if (get_suspend_state()==0){
                            ...
                            }else if ((get_suspend_state())&&(ddata->keycode==KEY_F5)){
                                input_event(ddata->input, EV_KEY, KEY_WAKEUP, 1);
                                input_sync(ddata->input);
                            }
                            //printk("0\n");
              //kairong20131221              input_event(ddata->input, EV_KEY, ddata->keycode, 1);
              //kairong20131221              input_sync(ddata->input);
...
void remotectl_wakeup(unsigned long _data)
{
    ...
    if (get_suspend_state()){
        ...
        for (i=0;i<ddata->remotectl_suspend_data.cnt;i++){
        		if (ddata->count>=32)
        			break;
			printk("TIME_BIT1_MIN = %d\n", TIME_BIT1_MIN);
			printk("time[%d] = %d\n", i, time[i]);
			printk("TIME_BIT1_MAX = %d\n", TIME_BIT1_MAX);
           if ((TIME_BIT1_MIN < time[i]) && (time[i] < TIME_BIT1_MAX)){
//               printk("before |= 0x01, ddata->scanData = %d\n", ddata->scanData); 
			   ddata->scanData |= 0x01;
//               printk("after |= 0x01, ddata->scanData = %d\n", ddata->scanData); 
                ddata->scanData <<= 1;
//               printk("after <<= 1, ddata->scanData = %d\n", ddata->scanData); 
                ddata->count ++;;
//				printk("ddata->count = %d\n", ddata->count);
...
//        printk("yyyyyyyyyyyy ddata->scanData = 0x%x\n", ddata->scanData);
		if (ddata->scanData)					//(ddata->scanData>16)			
				{
//					printk("aaaaaaaaaaaa ddata->scanData = 0x%x\n", ddata->scanData);
					  ddata->scanData=(ddata->scanData>>1)&0xFFFF;				
					  printk(KERN_ERR"data=0x%x\n",ddata->scanData);
					  
					  for (i=0;i<sizeof(remotectl_button)/sizeof(struct rkxx_remotectl_button);i++){
					  	remotectl_get_pwr_scanData(ddata,&power_scanData,i);
						printk("power_scanData = 0x%x\n", power_scanData);
						printk("ddata->scanData&0xffff = 0x%x\n", ddata->scanData&0xffff);
						printk("ddata->scanData&0x0fff = 0x%x\n", ddata->scanData&0x0fff);
						printk("ddata->scanData&0x00ff = 0x%x\n", ddata->scanData&0x00ff);
